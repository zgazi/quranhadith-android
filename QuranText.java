...
public class QuranText {
	...
	public QuranText(InputStream inStream, boolean isArabic)
	{
		...
		if(inStream==null){
			Log.i("QuranText", "inputstream not found");
			return;
		}
		...
		try
		{
			...
			int suraIndex=0;
			int ayahRead=0;
			int ayahCount=Sura.ayahCounts[0];
			...
			while((text=reader.readLine())!=null)
			{
				...
				if(ayahRead==ayahCount)
				{
					...
					suraIndex++;
					ayahRead=0;
					ayahCount=Sura.ayahCounts[suraIndex];
					quranText[suraIndex]=new String[ayahCount];
				}
			}
			Log.i("QuranText", "quran Text loading success");
			reader.close();
		}
	...
	public String getQuranText(int sid, int vid) {
		return quranText[sid][vid];
	}
	...
	public void search(String query, int maxSearchCount,
	    ...
	    for(int i=startSuraIndex;i<=endSuraIndex;i++){
	    	for(int j=0,k=Sura.ayahCounts[i];j<k;j++){
